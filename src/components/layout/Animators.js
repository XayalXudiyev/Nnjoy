...
const AutoPlayCarousel = () => {
    ...
    return (
        <div className='p-4'>
            <div className="d-flex align-items-center justify-content-between "><h1>Animatorlar</h1>
            ...
            <Modal isOpen={modalIsOpen} onRequestClose={handleModalClose} >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gridGap: '20px',  padding: '30px', }}>
                    {images.map((image, index) => (
                        <div key={index}>
                            <img src={image.url} alt={image.alt} style={{ width: '100%' , borderRadius:"10px"}} />
                        </div>
                    ))}
                </div>
            </Modal>
